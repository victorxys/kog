...
<body>
	...
	<div id="full" class="content" style="max-width: 600px; text-align: center;"> 
		<div class="row-full">日期:<?php _e(date('Y-m-d H:i',$game->start_time).$finished_time)?></div>
		<div><!-- 座位图 -->
			

					<?php require_once('kog_table.php');?>
					...
					if (isset($rebuy_real[$val->uid]['killed_by'])) {
						if (is_array($rebuy_real[$val->uid]['killed_by'])) {
							...
							foreach ($rebuy_real[$val->uid]['killed_by'] as $k => $v) {
								...
								$killed_by_player = $game_detail_position[$v]->player;

							}
						} else {
							echo $game_detail_position[$v]->player;
						}
					} else {
						echo "--";
					}
					...
					<div class="cell1-5-heigh">
						<div class="div-input-left-end">
							...
							<?php erp_html_form_input( array(
								...
								'type'    	=> 'text',
							// 'function' 	=> 'onkeyup =value=value.replace(/[^\d\.]/g,'');SumNum(this.value);\"',
