...
// count_ip_duan();
function count_ip_duan(){
	global $wpdb;
	$sql = "select IP from fromexcel";
	$res = $wpdb->get_col($sql);
	foreach ($res as $key => $value) {
		$ip_duan	= 	substr($value,0,strrpos($value,"."));
		$ip_duan_arr[$ip_duan] = $ip_duan;

	}
	foreach ($ip_duan_arr as $key => $value) {
		$sql_insert = "INSERT INTO `ipduan` (`ipduan`,`laiyuan`) values ('".$value."','Excel')";
...
function get_ip_all_ipduan($source=null){
	global $wpdb;
	$sql = "select ipduan from ipduan where 1=1 ";
	if (!is_null($source)) {
		$sql = $sql." and laiyuan = '".$source."'";
	}
	$sql = $sql." order by ipduan";
	$res = $wpdb->get_col($sql);
	// 	echo "<pre>";
	// var_dump($res);
	// exit;
...
/* 
 根据基本信息获取ip信息
*/
function get_ip_info($jifang=null, $from, $other=null, $kehu=null, $ip=null, $shuxing=null, $kehuid=null){
	global $wpdb;
	$sql = "SELECT * FROM "."from".$from ." where 1=1 " ;
	if (!is_null($jifang)) {
		$sql = $sql." and jifang = '".$jifang."'";
	}
	if (!is_null($kehu)) {
	...
	if (!is_null($shuxing)) {
		$sql = $sql." and shuxing = '".$shuxing."'";
	}
	if (!is_null($kehuid)) {
		$sql = $sql." and kehuid = '".$kehuid."'";
	}
	if (!is_null($other)) {
		$sql = $sql." and other = '".$other."'";
	}
	$sql = $sql." order by ip";
...
function get_ip_by_ipduan($ip_duan,$from){
	global $wpdb;
	$sql = "SELECT * FROM "."from".$from ." where 1=1 and ip like '".$ip_duan."%' order by ip" ;
	$res = $wpdb->get_results($sql);
	return $res;
	// 	echo "<pre>";
	// var_dump($sql);
...
function get_ip_duan($arr_ip){
	...
	// exit;			
			// 找出ip段
			$ip_duan	= 	substr($value->IP,0,strrpos($value->IP,"."));
			// 计算IP段中ip数量
			$ip_duan_arr[$ip_duan]['count'] = isset($ip_duan_arr[$ip_duan]['count'])?$ip_duan_arr[$ip_duan]['count']+1:1;
			// 显示ip
			$ip_duan_arr[$ip_duan]['ip'][$key]=$value->IP;
			if ($value->zhuangtai == "1") {
				$ip_duan_arr[$ip_duan]['used'] = isset($ip_duan_arr[$ip_duan]['used'])?$ip_duan_arr[$ip_duan]['used']+1:1;

			}
			// $ip_duan_arr['all_ip'][$value->ID]=$value->IP;
			$ip_duan_arr['all_ip'][$key]=$value->IP;
		}
...
function check_if_ip($ip,$ip_source){
	$res = array_search($ip,$ip_source);
		// echo "<pre>";
		// var_dump($res);
		// // exit;
...
function check_if_same($source,$target){
	// echo "<pre>";
	// 	var_dump($target);
	// 	exit;
	if ($source != $target) {
		return "<span class='wrong'>".$source."</span>";
	}
	else{
		return $source;
	}
...
function check_zhuagntai($source,$target){
	if ($source == "已预留" || $source =="已预占") {
		if ($target == "1") {
			return $source;
		}else{
			return "<span class='wrong'>".$source."</span>";
...
// check_if_like("双线BGP","北京兆维双线BGP IP");
// 分词，找每个词是否在目标中有，都有就算匹配
function check_if_like($source,$target){

	$target_array = str_split_unicode($target);
	// echo "<pre>";
	...
	foreach ($target_array as $key => $value) {
		...
		// // echo $value;
		// echo "---".$source;
		// var_dump(mb_strrpos($source, "0", 'UTF8'));
		// exit;
		if(mb_strrpos($source, $value, 'UTF8') === false){
			return "<span class='wrong'>".$source."</span>";
		}
	}
	return $source;
...
function check_if_empty($source,$target){
...
function creat_err_log($key,$err_type){
...
function get_player($uid = null,$start_time=null, $end_time=null, $game_type=''){
	global $wpdb;
